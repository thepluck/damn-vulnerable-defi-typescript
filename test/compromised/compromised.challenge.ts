...
describe('Compromised challenge', function () {
  it('should obtain all ETH available in the exchange', async function () {
    ...
    const oracleInitializer = await (
      await ethers.getContractFactory('TrustfulOracleInitializer', deployer)
    ).deploy(sources, ['DVNFT', 'DVNFT', 'DVNFT'], [INITIAL_NFT_PRICE, INITIAL_NFT_PRICE, INITIAL_NFT_PRICE]);
    ...
    ).deploy(oracle, { value: EXCHANGE_INITIAL_ETH_BALANCE });
